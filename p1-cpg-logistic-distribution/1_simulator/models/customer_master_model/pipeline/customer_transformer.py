...
import string
import pandas as pd
from config.customers_params import GEOFENCE_RADIUS_M, SEGMENT_DISTRIBUTION
from utils.geo import add_city_and_state
from models.warehouse_clustering import run_clustering as _assign_region
...
def transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_customer, df_warehouses = _assign_region(
                                df=df,
                                save_plot=True)

    df_warehouses['warehouse_code'] = df_warehouses['warehouse_name'].str.split(' · ').str[0]
    df_warehouses = df_warehouses[['warehouse_id', 'warehouse_code','warehouse_name','lat','lon']]

    df_customer['warehouse_code'] = df_customer['warehouse'].str.split(' · ').str[0]
    df_customer["geofence_radius_m"] = GEOFENCE_RADIUS_M
    df_customer = df_customer[['name','brand','opening_hours','lat','lon','city','state','customer_id','segment','geofence_radius_m', 'warehouse_code']]
    
    logger.info("Transform complete. DF size: %d", len(df_customer))
    return df_customer, df_warehouses
